// $Id: ReadSetScaling.java,v 1.6 2023/11/02 16:36:16 kingc Exp $
...
class ReadSetScaling implements Scaling
{
	...
	static final int COMMON_MAX = 90;
	...
	static final int SHRT_MIN = -32768;

	static final int SHRT_MAX = 32767;
	...
	static final double lowlim[] = { -10.24, -10.0, -5.0, -2.5, 0.0, -4.295E9,
	...
	static final double uprlim[] = { 10.235, 9.995, 4.998, 2.499, 65536.0,
	...
	final int pIndex, cIndex;
	final int scalingLength;
	final String pUnits, cUnits;
	final int displayFormat, displayLength;
	final double[] constants;

	ReadSetScaling(int di, int pi, Lookup_v2.ReadSetScaling scaling)// throws AcnetStatusException
	{
		this.di = di;
		this.pi = pi;
		this.pIndex = scaling.primary.index;
		this.cIndex = scaling.common.index;
		this.scalingLength = scaling.primary.inputLen;
		this.pUnits = scaling.primary.units;
		this.cUnits = scaling.common.units;
		this.displayFormat = scaling.displayFormat == Lookup_v2.DisplayFormat.Normal ? 1 : 0; 
		this.displayLength = scaling.displayLength == Lookup_v2.DisplayLength.Short ? 6 : 0;
		this.constants = scaling.common.constants;
	}

	private int commonConstCount(int index) 
	{
		switch(index) {
		// X' = X
		case 0:
	...
	{
		return primaryToCommon(rawToPrimary(data));
	}
	...
	double rawToPrimary(int data) throws AcnetStatusException 
	{
		int int_temp = 0;
		int i = 0;
		int x = 0;
		...
		case 54:
			if ((scalingLength != 2) || (((short) data & 0x8000) != 0)) {
			 ...
			 */
		case 56:
			if (scalingLength != 2) { /* must be a two byte quantity */
		...
		case 84:
			// Again with the extra bitmasks to fix results
			if (scalingLength != 4) { /* must be a 4 byte quantity */
				throw new AcnetStatusException(DIO_SCALEFAIL, "PDB data length error - must be two byte quantity.");
			}
			 ...
			 */
		default:
			throw new AcnetStatusException(DIO_SCALEFAIL, "Primary transform " + pIndex + " not found.");
		}
	}
	 ...
	 */
	private double primaryToCommon(double data, int index, double[] c, int num_constants) throws AcnetStatusException 
	{
		int il;
		double x = 0.0;
		double flt_data = 0.0;
...
//		num_constants = constants.length;
		...
		/*
		 * * Process to common units where * X' = X
		 */
		case 0:
			flt_data = x;
			return flt_data;
...
//					+ (c[0] * Math.pow(x, 5.0));
		...
		case 56:
			if (num_constants < 3)
				throw new AcnetStatusException(DIO_SCALEFAIL,
						"Insufficient number of constants provided in PDB.");
			flt_data = interpolate_table((int) c[0],
					new double[] { c[1], c[2] },
    ...
    ******************************************************************************/
    private double mfcTransform(double data) throws AcnetStatusException
	{
		...
		for (il = table_pointer; il < table_pointer+n_of_diapasons; il++) {
			...
			flt_data  = primaryToCommon( x, jl, ScalingInternal.all_transform_values[il].cx, commonConstCount(jl) );
			break;
			}
			}
	...
	/**
	 * Scales the value of a device from primary units to common units.
	 * 	for all common transforms including #90
	 * @param data
	 *            double primary value to scale to common units
	 * @return a scaled value (in common units)
	 * @exception AcnetStatusException
	 *                on scaling error
	 */
	double primaryToCommon(double data) throws AcnetStatusException 
	{
		return cIndex == 90 ? mfcTransform(data) :
								primaryToCommon(data, cIndex, constants, constants.length);
	}

	@Override
	public double scale(byte[] data, int offset) throws AcnetStatusException
	{
		return primaryToCommon(rawToPrimary(raw(data, offset, scalingLength)));
	}

	@Override
	public byte[] unscale(double data, int length) throws AcnetStatusException
	{
		return raw(primaryToRaw(commonToPrimary(data)), length);
	}
	...
	/** *************************************************************************** */
	/**
	 * check_signed_overflow Internal routine to check for a signed integer
	 * overflow
	 */
	/** *************************************************************************** */

	static private boolean check_signed_overflow(int value, int size)
	...
	static private boolean checkOverflow(int value, int size) //throws AcnetStatusException 
	{
		switch (size) {
		case 1:
	...
	/** *************************************************************************** */
	/*
	 * check_unsigned_overflow Internal routine to check for an unsigned integer
	 * overflow
	 */
	/** *************************************************************************** */

	static private boolean check_unsigned_overflow(int value, int size) 
	{
		switch (size) {
		case 1:
			if ((value < 0) || (value > UCHAR_MAX))
	 ...
	 *                on scaling error
	 */
	double commonToPrimary(double xx, int index, double [] c, double prim_low,
			    double prim_upp ) throws AcnetStatusException
	{
		double state;
		double tmpx;
		...
		if ((prim_low == 0)&&(prim_upp == 0))
		    {		/* default values */
		    pulow = lowlim[pIndex/2];
		    puupr = uprlim[pIndex/2];
		    }
		else
		...
		if (index != 201) {	//for the case 90 - no index control
			if ((pIndex > PRIMARY_MAX) || (index > COMMON_MAX))
		...
		case 12:
			if ((c[0] == 0.) && (c[1] == 0.) && (c[2] != 0.)) {
				...
				pumid = binary_search_loop(tol, pulow, puupr, xx);
				return pumid;
			}
		...
		case 38:
			...
			if (pIndex == 16) {		/* raw floating point number */
			...
			return (root_bisection(xx, pulow, puupr, tol));
		...
		case 56:
			...
			state = (float) reverse_interpolate_table((int) c[0], new double[] {
		...
		case 64:
			...
			if ((xx < 0) || (xx > primaryToCommon(puupr)))
				throw new AcnetStatusException(DIO_SCALEFAIL, "Value is out of range");
			pumid = binary_search_loop(tol, pulow, puupr, xx);
			return (pumid);
		...
		case 68:
			...
			if ((xx < 0) || (xx > primaryToCommon(pulow)))
				throw new AcnetStatusException(DIO_SCALEFAIL, "Value is out of range");
			pumid = binary_search_loop(tol, pulow, puupr, xx);
			return pumid;
    ...
    /******************************************************************************
    * reverse_mfc_transform: multifunction common transform
    *	This function is called to do the actual scaling of 
    *       data from common to primary units for the case 90
    *
    ******************************************************************************/

    double reverse_mfc_transform(double data) throws AcnetStatusException {
	...
	for (il = table_pointer; il < table_pointer+n_of_diapasons; il++) {
		...
		/* the diapason has been found, proceed to the conversion */
		jl = ScalingInternal.all_transform_values[il].common_transform;
		flt_data  = commonToPrimary( x, jl, ScalingInternal.all_transform_values[il].cx,
    ...
    }		/* end of reverse_mfc_transform() */

	/**
	 * Scales the value of a device from common units to primary units.
	 * 	for all common transforms including #90
	 * @param data
	 *            double common value to scale to primary units
	 * @return a scaled value (in common units)
	 * @exception AcnetStatusException
	 *                on scaling error
	 */
	double commonToPrimary(double data) throws AcnetStatusException 
	{
		double common_value = data;
		...
		switch (cIndex) {
			...
			//
		    case 90:
	        	primary_value = reverse_mfc_transform(common_value);
			break;
		    default:
			primary_value = commonToPrimary(common_value, cIndex, constants, 0, 0 );
			break;
		    }
	...
	/******************************************************************************/
	/* binary_search_loop
	 *	This routine finds an inverted value for a transform by using the
	 *	method of approaching by consecutive approximations dividing by 2.
	 *       The regular situation when the method works is the different signs
	 *	(f(a) < xx < f(b), or (f(a) > xx > f(b), interval [a,b] )
	 *       of the trunsform function at the ends of the initial interval.
	 *       If it is not so we are looking at first for such an interval
	 */
	/******************************************************************************/
	private double binary_search_loop(double tol, double pul, double puu, double xx) throws AcnetStatusException 
	{
		...
		cuupr = primaryToCommon(puupr) - xx;
		/* scale lower  limit to common units */
		culow = primaryToCommon(pulow) - xx;
		...
		while (true) {
			...
			if (((cuupr > 0) && (culow > 0)) || ((cuupr < 0) && (culow < 0))) {
				pumid = (pulow + puupr) / 2.;
				cumid = primaryToCommon(pumid) - xx;
		...
		while (true) {
			...
			/* scale upper limit to common units */
			cuupr = primaryToCommon(puupr);

			/* scale midpoint to common units */
			cumid = primaryToCommon(pumid);
	...
	/** *************************************************************************** */
	/**
	 * root_bisection This routine finds an inverted value for a transform that
	 * cannot be symbolically solved using the bracketed bisection method.
	 */
	/** *************************************************************************** */

	private double root_bisection(double desired, double x1, double x2, double xacc) 
									throws AcnetStatusException 
	{
		int j;
		double dx, f, fmid, xmid, rtb;

		f = primaryToCommon(x1) - desired;

		fmid = primaryToCommon(x2) - desired;
		...
		for (j = 1; j <= MAX_ITERATIONS; j++) { // iterate until the result is in tolerance
			xmid = rtb + (dx *= 0.5);
			fmid = primaryToCommon(xmid) - desired;
			if (fmid <= 0.0)
				rtb = xmid;
	...
	/******************************************************************************  
	 *
	 * Emulate CLIB ICSHFT function.
	 */
	/** ************************************************************************* */
	static private short ICSHFT(short value, short nbits) 
	{
		long x = 0, y = 0;
	...
	/** *************************************************************************** */
	/**
	 * Emulate macro from scaling C code: #define mod(x,y) x- x/y*y
	 */
	/** *************************************************************************** */
	 ...
	 *                on scaling error
	 */
	int primaryToRaw(double x) throws AcnetStatusException 
	{
		short size = (short) scalingLength;
		float xx;
		int x1 = 0, x2 = 0;
		long y = 0;
		short expon;
		...
		case 22: /* VAX/VMS null transform -- convert to DEC float */
			state = Float.floatToIntBits(xx * 4); // DEC float = 4*IEEE float
		...
		case 44: /* a decimal encorded into 7-digit BCD format */
			...
			state = 0;
			for (i = 7; i >= 1; i--) {
				hbit = (short) (temp / Math.pow(10, i));
				if (hbit != 0) {
					temp = mod(temp, (int) Math.pow(10, i));
					state = (int) (state + hbit * Math.pow(16, i));
				}
			}
			state = state + temp;
			return (state);
		...
		case 48: /* temperature resistors */
			...
			state = Float.floatToIntBits(xx * (float) 0.036);
			return (state);

		case 52: /* byte-swap word or longword */
			state = (int) xx;
			if (check_signed_overflow(state, size)) {
				if (size == 2) {// Byte swap raw data
		...
		case 60: /* temperature resistors */
			state = Float.floatToIntBits(xx / (float) 500.0);
			return (state);
		...
		case 72: /* -/+ 10.0V . 0000-FFFF */
			 ...
			 */
		case 74:
			if ((size != 2) || (xx < -21.0) || (xx > 21.0)) {
				throw new AcnetStatusException(DIO_SCALEFAIL, "Data size or data range invalid.");
			}
	...
	/** *************************************************************************** */
	/*
	 * interpolate_table Internal function to perform a linear or exponential
	 * interpolation of a scaling table
	 * 
	 * table_number - scaling table identifier passed_absolute_limits - absolute
	 * limits of the primary units values from the PDB (If both values are zero,
	 * use the limits from the scaling table.) type - type of interpolation
	 * (linear or exponential) primary_value - primary units value to scale (eg.
	 * in volts) common_value - returned scaled value in common units
	 */
	/** *************************************************************************** */
	static double interpolate_table(int table_number, double[] passed_absolute_limits, int type, double primary_value)
			throws AcnetStatusException 
	{
		int tbl;
	...
	/** *************************************************************************** */
	/*
	 * reverse_interpolate_table Internal function to reverse a linear or
	 * exponential interpolation of a scaling table
	 * 
	 * table_number - scaling table identifier passed_absolute_limits - absolute
	 * limits of the primary units values from the PDB (If both values are zero,
	 * use the limits from the scaling table.) type - type of interpolation
	 * (linear or exponential) common_value - common units value to scale
	 * primary_value - returned primary units value (eg. in volts)
	 */
	/** *************************************************************************** */

	static double reverse_interpolate_table(int table_number,
			double[] passed_absolute_limits, int type, double common_value)
			throws AcnetStatusException 
	{
